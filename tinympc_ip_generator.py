...
Generates complete HLS IP core with unified memory interface as described in goal.md
"""
...
void update_linear_cost(
    ...
    def generate_main_solver(self):
        ...
        # Generate hardcoded constraint arrays
...
// Hardcoded constraints as ROM (constant arrays)
...
#pragma HLS PIPELINE off
    ...
    def generate_test_data(self):
        ...
        x0_track = np.zeros(self.solver.nx, dtype=np.float32)
        ...
        for case in test_cases:
            ...
            # Disable check termination to run fixed iterations
            original_check = self.solver.check_termination
            self.solver.check_termination = 0  # Disable termination check
            
            # Run for fixed number of iterations
            self.solver.solve()
            
            # Restore original setting
            self.solver.check_termination = original_check
            
            # Collect results
            results.append({
                'x_out': self.solver.vnew.copy(),
                'u_out': self.solver.znew.copy(),
                'final_iter': self.solver.iter
            })
    ...
    def generate_subfunctions(self):
        ...
        from hls_generator_v2 import TinyMPCFunctionGenerator
...
int main() {{
    ...
    for (int test = 0; test < NUM_TESTS; test++) {{
        ...
        for (int k = 0; k < N; k++) {{
            for (int i = 0; i < NX; i++) {{
                x_out[k][i] = main_memory[X_OUT_OFFSET + k*NX + i];
            }}
        }}
        test_passed &= validate_arrays((data_t*)x_out, (data_t*)expected_x_out[test], N*NX, "x_out");
        
        // Extract and validate u_out
        data_t u_out[N_MINUS_1][NU];
        for (int k = 0; k < N_MINUS_1; k++) {{
            for (int j = 0; j < NU; j++) {{
                u_out[k][j] = main_memory[U_OUT_OFFSET + k*NU + j];
            }}
        }}
        test_passed &= validate_arrays((data_t*)u_out, (data_t*)expected_u_out[test], N_MINUS_1*NU, "u_out");
